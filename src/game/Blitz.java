...
public class Blitz implements Game, Runnable {
    ...
    private final String ICON_PLAYER             =     "ct/sm_ct_ehren-smiley-ani.gif";
   // private final String ICON_PLAYER             =     "nicklistblitz.gif";
    ...
    private final int    WIN_KNUDDELS               =     2; // Knuddels die der Sieger pro Runde erhält.
    private final int    MIN_KNUDDELS               =     1; // Der Einsatz an Knuddels pro Runde.
        ...
        while (_request || _playing) {
            ...
            if (_playing) {
                ...
                if (_activeClients.size() == 1) {
                    ...
                    int winKnuddels = _roundNumber * WIN_KNUDDELS;
                    client.increaseKnuddels(winKnuddels);
                    _channel.broadcastButlerMessage(String.format(
                              "°+0065>mychannel/ico_dice-6_001.gif<°°+0100°°18°°BB°_%s hat das Spiel gewonnen_.°r°#"
                              + "%s bekommt für den Sieg °16BB°_%d_°r° Punkte und °16BB°_%d_°r° Knuddels."
                            , getClickableNickString(_activeClients.get(0).getName()), client.getGenderLabel(),winPoints, winKnuddels));
    ...
    private void addClient(Client pClient, boolean pOut) {
        ...
        if (pOut) {
            pClient.sendButlerMessage(_channel.getName(), "Du bist nun _für Blitz! angemeldet_. Ich habe einen Knuddel bei dir abgezogen.");
            pClient.deseaseKnuddels(MIN_KNUDDELS);
            if(pClient.getKnuddels() < 1) {
                   pClient.sendButlerMessage(_channel.getName(), String.format("Du hast nicht genug Knuddels.", pClient.getKnuddels()));
                   return;
                }
        }
    ...
    public boolean parsePublicMessage(String pMessage, Client pClient) { 
        ...
        if (_request) {
          ...
          //  int w = Integer.parseInt(arg);
           // int k = (w*_channel.getMinknuddels());
